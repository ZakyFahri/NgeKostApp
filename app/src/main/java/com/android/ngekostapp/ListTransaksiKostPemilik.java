...
public class ListTransaksiKostPemilik extends AppCompatActivity {
    ...
    private ListTransaksiKostPemilikAdapter listTransaksiKostPemilikAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_list_transaksi_kost_pemilik);
        ...
        //Widgets
        recyclerView =findViewById(R.id.ListTransaksiPemilik);
    ...
    //BottomSheetDialog
    private void ViewFilter(String namaPemilik){
        ...
        filterdata.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(RadioGroup radioGroup, int i) {
                ...
                if (infoTransaksi.getText().toString().contains("Transaksi : Menunggu Konfirmasi")){
                    db.collection("Data Transaksi").whereEqualTo("namaPemilik", namaPemilik).whereEqualTo("statusTransaksi", "MENUNGGU KONFIRMASI").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                        @Override
                        public void onComplete(@NonNull Task<QuerySnapshot> task) {
                            ...
                            setOnClickListener();
                            listTransaksiKostPemilikAdapter = new ListTransaksiKostPemilikAdapter(ListTransaksiKostPemilik.this, daftarTransaksi, listener);
                            recyclerView.getRecycledViewPool().clear();
                            recyclerView.setAdapter(listTransaksiKostPemilikAdapter);
                            listTransaksiKostPemilikAdapter.notifyDataSetChanged();
                            bottomSheetDialog.dismiss();
                        }
                    });
                }else if (infoTransaksi.getText().toString().contains("Transaksi : Telah Dikonfirmasi")){
                    db.collection("Data Transaksi").whereEqualTo("namaPemilik", namaPemilik).whereEqualTo("statusTransaksi", "TELAH DIKONFIRMASI").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                        @Override
                        public void onComplete(@NonNull Task<QuerySnapshot> task) {
                            for (DocumentSnapshot doc: task.getResult()){
                                ListTransaksiKost daftarlisttransaksi = doc.toObject(ListTransaksiKost.class);
                    ...
                    db.collection("Data Transaksi").whereEqualTo("namaPemilik", namaPemilik).whereEqualTo("statusTransaksi", "TRANSAKSI SELESAI").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                        @Override
                        public void onComplete(@NonNull Task<QuerySnapshot> task) {
                            for (DocumentSnapshot doc: task.getResult()){
                                ListTransaksiKost daftarlisttransaksi = doc.toObject(ListTransaksiKost.class);
                    ...
                    db.collection("Data Transaksi").whereEqualTo("namaPemilik", namaPemilik).whereEqualTo("statusTransaksi", "TRANSAKSI DITOLAK").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                        @Override
                        public void onComplete(@NonNull Task<QuerySnapshot> task) {
                            for (DocumentSnapshot doc: task.getResult()){
                                ListTransaksiKost daftarlisttransaksi = doc.toObject(ListTransaksiKost.class);
    ...
    private void setOnClickListener(){
        listener = new ListTransaksiKostPemilikAdapter.RecylerViewClickListener() {
            @Override
            public void onClick(View view, int position) {
                Intent keDetailTransaksi = new Intent(getApplicationContext(), DetailTransaksiPemilik.class);
    ...
    private void loadDataTransaksi(String namaPemilik){
        collectionReference.whereEqualTo("namaPemilik", namaPemilik).orderBy("namaKost", Query.Direction.ASCENDING).get().addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
            @Override
            public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                if(!queryDocumentSnapshots.isEmpty()){
                    ...
                    Toast.makeText(ListTransaksiKostPemilik.this, "Data Transaksi Tidak Ada", Toast.LENGTH_LONG).show();
                }
            }
        });
    }
